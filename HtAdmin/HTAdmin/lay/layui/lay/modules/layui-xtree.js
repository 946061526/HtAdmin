...
layuiXtree.prototype.dataBind = function (d) {
    ...
    if (d.length > 0) {
        for (i in d) {
            ...
            var xtree_visible = d[i].visible ? '' : ' hide';
            _this._domStr += '<div class="layui-xtree-item' + xtree_visible + '">';
...
// 选中全部 p
layuiXtree.prototype.SetAllCheck = function (data,chkParentNode) {
    var _this = this;
    var xtree_allck = chkParentNode.getElementsByClassName('layui-form-checkbox');
    for (var i = 0; i < xtree_allck.length; i++) {
        if (xtree_allck[i].getAttribute('class').indexOf('layui-checkbox-disbaled') == -1) {
            if (data.elem.checked) {
                xtree_allck[i].classList.add('layui-form-checked');
